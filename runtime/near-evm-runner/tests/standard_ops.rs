...
mod tests {
    ...
    use near_evm_runner::utils::{
        address_from_arr, address_to_vec, encode_call_function_args,
    ...
    fn test_funds_transfers() {
        ...
        assert_eq!(
            context
                .get_balance(address_to_vec(&near_account_id_to_evm_address(&accounts(1))))
                .unwrap(),
            U256::from(0)
        );
        let mut context =
        ...
        let mut context =
            create_context(&mut fake_external, &vm_config, &fees_config, accounts(1), 0);
        context
            .transfer(
                TransferArgs {
        ...
        assert_eq!(
            context
                .get_balance(address_to_vec(&near_account_id_to_evm_address(&accounts(2))))
                .unwrap(),
            U256::from(50)
        );
        let mut context =
            create_context(&mut fake_external, &vm_config, &fees_config, accounts(2), 0);
        context
            .withdraw(
    ...
    fn test_internal_create() {
        ...
        let new_raw =
            context.call_function(encode_call_function_args(sub_addr, new_input)).unwrap();
    ...
    fn test_solidity_accurate_storage_on_selfdestruct() {
        ...
        let input =
            create2factory::functions::test_double_deploy::call(salt, destruct_code.clone()).0;
